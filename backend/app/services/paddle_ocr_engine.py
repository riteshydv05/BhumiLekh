...
# ---------------------------------------------------------------------------
_LANG = os.environ.get("PADDLE_OCR_LANG", "en")
...
def _get_engine() -> "_PaddleOCR | None":
    ...
    with _init_lock:
        ...
        logger.info(
            "PaddleOCR: initializing engine (lang=%s, angle_cls=%s)",
            _LANG, _USE_ANGLE,
        )
        try:
            _ocr_instance = _PaddleOCR(
                lang=_LANG,
                use_doc_orientation_classify=False,   # Skip heavy doc rotation
            ...
            logger.info("PaddleOCR: engine initialized successfully (PP-OCRv6, lang=%s)", _LANG)
...
def _parse_result(paddle_result) -> list[dict]:
    """Parse a single PaddleOCR 3.x result object into a list of block dicts.
    ...
    blocks: list[dict] = []
    try:
        res = paddle_result.json.get("res", {})

        texts = res.get("rec_texts", [])
        scores = res.get("rec_scores", [])
        polys = res.get("rec_polys", res.get("dt_polys", []))
        boxes = res.get("rec_boxes", [])  # [x1, y1, x2, y2] per text
        ...
        for i, text in enumerate(texts):
            ...
            # Prefer rec_boxes (axis-aligned) for simplicity; fall back to poly
            ...
            # Full polygon for downstream LayoutLM use
